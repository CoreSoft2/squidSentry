...
Redis.prototype.getCats = function(callback){

    var transaction = this.client.multi();

    transaction.keys(this.prefix + ':ips:*');
    ...
    var tmp = [];
        ...
        for(var i in results[0]){
            ...
            var catIPs = results[0][i].split(':')[2];
            tmp.push(catIPs)
...
Redis.prototype.getIpByCats = function(catsArr, ip, callback){
    var transaction = this.client.multi();


    for(var i in catsArr){
        transaction.sismember(this.prefix + ':ips:' + catsArr[i], ip);
    }
    var clbtest = false;
    transaction.exec(function(err, results){
        ...
        if(clbtest ? callback(catsArr[i]) : callback(false));
        //callback(false);

    });
};

Redis.prototype.searchRulebyCat = function(catsArr, callback){
    var transaction = this.client.multi();

    transaction.exists(this.prefix + ':rule:' + catsArr);
...
Redis.prototype.GetRulebyCat = function(ipRule, callback){
    var transaction = this.client.multi();

    transaction.get(this.prefix + ':rule:' + ipRule);
